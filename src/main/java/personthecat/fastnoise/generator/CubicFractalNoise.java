...
public class CubicFractalNoise extends CubicNoise {
    ...
    public CubicFractalNoise(final NoiseDescriptor cfg, final FractalFunction fractal) {
        super(cfg);
    ...
    public CubicFractalNoise(final NoiseDescriptor cfg) {
    ...
    public float getNoise(float x, float y) {
        int seed = this.seed;
        float sum = this.fractal.apply(this.singleCubic(seed, x, y));
        float amp = 1;
        ...
        for (int i = 1; i < this.octaves; i++) {
            ...
            amp *= this.gain;
            sum += this.fractal.apply(this.singleCubic(++seed, x, y)) * amp;
        }
    ...
    public float getNoise(float x, float y, float z) {
        int seed = this.seed;
        float sum = this.fractal.apply(this.singleCubic(++seed, x, y, z));
        float amp = 1;
        ...
        for (int i = 1; i < this.octaves; i++) {
            ...
            amp *= this.gain;
            sum += this.fractal.apply(this.singleCubic(++seed, x, y, z)) * amp;
        }
        return sum * this.fractalBounding;
    }
}
