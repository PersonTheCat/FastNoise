...
public class ValueFractalNoise extends ValueNoise {
    ...
    public ValueFractalNoise(final NoiseDescriptor cfg, final FractalFunction fractal) {
        super(cfg);
    ...
    public ValueFractalNoise(final NoiseDescriptor cfg) {
    ...
    public float getNoise(float x, float y) {
        int seed = this.seed;
        float sum = this.fractal.apply(this.singleValue(seed, x, y));
        float amp = 1;
        ...
        for (int i = 1; i < this.octaves; i++) {
            ...
            amp *= this.gain;
            sum += this.fractal.apply(this.singleValue(++seed, x, y)) * amp;
        }
    ...
    public float getNoise(float x, float y, float z) {
        int seed = this.seed;
        float sum = this.fractal.apply(this.singleValue(++seed, x, y, z));
        float amp = 1;
        ...
        for (int i = 1; i < this.octaves; i++) {
            ...
            amp *= this.gain;
            sum += this.fractal.apply(this.singleValue(++seed, x, y, z)) * amp;
        }
        return sum * this.fractalBounding;
    }
}
